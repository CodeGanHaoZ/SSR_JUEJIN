import o from "prop-types";
import e from "react";
import t from "styled-components";
import { Box as a } from "../Box/Box.js";
import { FocusTrap as c } from "../FocusTrap/FocusTrap.js";
import { Portal as f } from "../Portal/Portal.js";
import { Flex as g } from "../Flex/Flex.js";
import { Typography as y } from "../Typography/Typography.js";
import { DismissibleLayer as b } from "../DismissibleLayer/DismissibleLayer.js";
import { setOpacity as E } from "../helpers/setOpacity.js";
import h from "../helpers/useLockScroll.js";
import { useId as D } from "../helpers/useId.js";
const x = t(a)`
  inset: 0;
  background: ${({ theme: r }) => E(r.colors.neutral800, 0.2)};
`, w = t(a)`
  max-width: ${412 / 16}rem;
  margin: 0 auto;
  overflow: hidden;
  margin-top: 10%;
`, R = t(g)`
  border-bottom: 1px solid ${({ theme: r }) => r.colors.neutral150};
`, i = ({ onClose: r, title: m, as: s, isOpen: n, id: d, ...p }) => {
  const u = D(d);
  if (h(n), !n)
    return null;
  const l = `${u}-label`;
  return /* @__PURE__ */ e.createElement(f, null, /* @__PURE__ */ e.createElement(x, {
    padding: 8,
    position: "fixed",
    zIndex: 4
  }, /* @__PURE__ */ e.createElement(c, null, /* @__PURE__ */ e.createElement(b, {
    onEscapeKeyDown: r,
    onPointerDownOutside: r
  }, /* @__PURE__ */ e.createElement(w, {
    "aria-labelledby": l,
    "aria-modal": !0,
    background: "neutral0",
    hasRadius: !0,
    shadow: "popupShadow",
    role: "dialog"
  }, /* @__PURE__ */ e.createElement(R, {
    padding: 6,
    justifyContent: "center"
  }, /* @__PURE__ */ e.createElement(y, {
    variant: "beta",
    as: s || "h2",
    id: l
  }, m)), /* @__PURE__ */ e.createElement(a, {
    ...p
  }))))));
};
i.displayName = "Dialog";
i.defaultProps = {
  as: "h2",
  id: void 0
};
i.propTypes = {
  as: o.string,
  id: o.oneOfType([o.string, o.number]),
  isOpen: o.bool.isRequired,
  onClose: o.func.isRequired,
  title: o.string.isRequired
};
export {
  i as Dialog
};
